...
from dask.distributed import Event, Pub, Sub
...
import asyncio
from pyblazing.apiv2.comms import Communicator
import json
import collections
...
# this needs to be called within the same event loop if repeatedly called from
# the same worker
@asynccontextmanager
async def communicate(workers):
    ...
    try:
        ...
        ucx_address = await comm.get_ucx_address()

        try:
            # synchronize between workers
            ev_published = "published_"
            ev_subscribed = "subscribed_"

            Event(ev_published+worker.name).clear()

            # PubSub for addresses
            pub_name = "ucx_address"
            sub = Sub(pub_name)
            Event(ev_subscribed+worker.name).set()
            for w in workers:
                ev_name = ev_subscribed+w
                Event(ev_name).wait()

            pub = Pub(pub_name)
            content = {worker.address: ucx_address}
            print(content,flush=True)
            pub.put(content)

            Event(ev_published+worker.name).set()

            for w in workers:
                ev_name = ev_published+w
                Event(ev_name).wait()

            # clear events generated by this worker
            Event(ev_subscribed+worker.name).clear()
            print('Done with events',flush=True)

            # collect addresses from other workers
            addr_map = {}
            print('before Sub',flush=True)
            print('after Sub',flush=True)
            for w in workers:
                p = sub.get()
                print('sub',p,flush=True)
                addr_map.update(p)
            print("Address mapping: ", addr_map,flush=True)
        except Exception as e:
            print('Error synchronizing between workers: {}'.format(e),flush=True)
            raise

        # set up endpoints
        print('Before create_endpoints')
        await comm.create_endpoints(addr_map)
        # now we need to sync once more between the workers before we can start exchanging data

        # synchronize between workers
        ev_barrier = "barrier_"
        print('before Event({}).set()'.format(ev_barrier+worker.name),flush=True)
        Event(ev_barrier+worker.name).set()

        print('there {}'.format(ev_barrier+worker.name),flush=True)
        for w in workers:
            ev_name = ev_barrier+w
            print('waiting for {}'.format(ev_name),flush=True)

            # the wait is ONLY there to yield the asyncio loop to the concurrently
            # running UCX progress
            while not Event(ev_name).wait(timeout='0.1s'):
                await asyncio.sleep(0)
                pass

        # run the communication in the background
        loop = asyncio.get_running_loop()
        task = loop.create_task(comm.start())
        yield task
    except Exception as e:
        print('Failed to set up communication: {}'.format(repr(e)),flush=True)

    finally:
        # set up endpoints
        print('Barrier 2')
        ev_barrier = "barrier2_"
        Event(ev_barrier+worker.name).set()
        ...
        print("Done {}".format(worker.name), flush=True)
        await comm.stop(task)
        print("Flushed {}".format(worker.name), flush=True)
                        ...
                        async def main():
                            async with communicate(workers):
                                with ThreadPoolExecutor(max_workers=1) as executor:
                                    # run communication in background
                                    loop = asyncio.get_running_loop()
